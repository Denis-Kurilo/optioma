...
$mail_to = "webprogram2017@mail.ru"; // E-mail на который будут отправляться уведомления
...
if(trim($_POST['phone'])){
	if(mail($mail_to, "=?UTF-8?B?".base64_encode($mail_subject)."?=", $mail_message1."--", implode("\r\n", $mail_headers))){
		echo "1"; exit;
	}else{header($_SERVER['SERVER_PROTOCOL']." 503 Service Unavailable"); echo "Error";}
}else{header($_SERVER['SERVER_PROTOCOL']." 400 Bad Request"); echo "Phone required";}
?>